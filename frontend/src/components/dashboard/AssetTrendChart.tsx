...
import { Calendar, ChevronDown, Settings2, Maximize2, X } from 'lucide-react'
...
export function AssetTrendChart() {
  ...
  const { data: trend, isLoading } = useAssetTrend({
  ...
  const chartData = useMemo(() => {
    ...
    while (currentDate <= end) {
      ...
      if (item) {
        ...
        filledData.push({
          date: dateStr,
          total_value_krw: null as unknown as number,
          daily_pnl: 0,
          daily_pnl_percent: 0,
      ...
      <div className="fixed inset-0 z-50 bg-white flex flex-col">
        <div className="flex items-center justify-between p-6 pb-4 border-b">
          <div className="flex items-center gap-6">
            <div>
              <h3 className="text-2xl font-bold text-gray-900">포트폴리오 성장 (전체화면)</h3>
              <p className="text-gray-500">자산 가치 추이</p>
            </div>
            {trend?.data && trend.data.length > 0 && (
              <div className="flex items-center gap-6 text-sm">
                <div>
                  ...
                  <span className={cn(
                    'font-semibold',
                    trend.period_return_percent >= 0 ? 'text-red-600' : 'text-blue-600'
                  )}>
